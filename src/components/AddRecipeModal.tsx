...
export function AddRecipeModal({ onClose, editRecipe }: Props) {
  ...
  const inputClass = 'w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-300 focus:border-orange-300 bg-white';
    ...
    >
      <div className="bg-white w-full sm:max-w-xl sm:rounded-2xl rounded-t-3xl shadow-2xl max-h-[92vh] flex flex-col animate-slide-up">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 flex-shrink-0">
          <h2 className="text-lg font-bold text-gray-900">{isEdit ? 'Edit Recipe' : 'Add Your Recipe'}</h2>
        ...
        <form onSubmit={handleSubmit} className="overflow-y-auto flex-1 min-h-0 px-6 py-4 space-y-5">
          {/* Emoji picker */}
          <div>
            <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider block mb-2">Pick an Emoji</label>
            <div className="flex flex-wrap gap-2">
              {EMOJIS.map((e) => (
                <button
                  ...
                  type="button"
                  onClick={() => set('emoji', e)}
                  className={`text-2xl p-1.5 rounded-xl transition-all ${form.emoji === e ? 'bg-orange-100 ring-2 ring-orange-400 scale-110' : 'hover:bg-gray-100'}`}
                >
                  {e}
                </button>
              ))}
          ...
          <div className="grid grid-cols-1 gap-3">
            <div>
              <label className="text-xs font-semibold text-gray-500 block mb-1">Recipe Name *</label>
            ...
            <div>
              <label className="text-xs font-semibold text-gray-500 block mb-1">Spanish Name</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="text-xs font-semibold text-gray-500 block mb-1">Cook Time</label>
              <input value={form.cookTime} onChange={(e) => set('cookTime', e.target.value)} className={inputClass} placeholder="e.g. 30 min" />
            </div>
            <div>
              <label className="text-xs font-semibold text-gray-500 block mb-1">Prep Time</label>
              <input value={form.prepTime} onChange={(e) => set('prepTime', e.target.value)} className={inputClass} placeholder="e.g. 15 min" />
            </div>
            <div>
              <label className="text-xs font-semibold text-gray-500 block mb-1">Servings</label>
          ...
          {/* Spice level */}
          <div>
            <label className="text-xs font-semibold text-gray-500 block mb-2">Spice Level</label>
            <div className="flex gap-2">
              {([1, 2, 3] as const).map((l) => (
                <button key={l} type="button" onClick={() => set('spiceLevel', l)}
                  className={`flex-1 py-2 rounded-xl text-sm font-medium border transition-all ${form.spiceLevel === l ? 'bg-orange-500 text-white border-orange-500' : 'bg-white text-gray-600 border-gray-200 hover:border-orange-300'}`}>
                  {'🌶️'.repeat(l)}
                </button>
              ))}
            </div>
          ...
          {/* Ingredients */}
          <div>
            <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider block mb-2">Ingredients</label>
            {form.ingredients.map((ing, i) => (
              <div key={i} className="flex items-center gap-2 mb-1.5 bg-orange-50 rounded-lg px-3 py-1.5">
                <span className="text-sm text-gray-700 flex-1 capitalize">{ing.name}</span>
                <span className="text-xs text-orange-600 font-medium">{ing.amount} {ing.unit}</span>
                <button type="button" onClick={() => set('ingredients', form.ingredients.filter((_, j) => j !== i))} className="text-gray-400 hover:text-red-500">
                  <Trash2 size={13} />
                </button>
              </div>
            ))}
            <div className="flex gap-2 mt-2">
              ...
              <button type="button" onClick={addIngredient} className="p-2 bg-orange-500 text-white rounded-xl hover:bg-orange-600 transition-colors flex-shrink-0">
                <Plus size={16} />
              </button>
            </div>
          </div>
          ...
          {/* Toppings */}
          <div>
            <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider block mb-2">Toppings</label>
            <div className="flex flex-wrap gap-1.5 mb-2">
              {form.toppings.map((t, i) => (
                <span key={i} className="flex items-center gap-1 text-xs bg-orange-100 text-orange-700 px-2 py-1 rounded-full">
                  {t}
            ...
            <div className="flex gap-2">
              ...
              <button type="button" onClick={addTopping} className="p-2 bg-orange-500 text-white rounded-xl hover:bg-orange-600 transition-colors flex-shrink-0">
                <Plus size={16} />
              </button>
            </div>
          </div>
          ...
          {/* Instructions */}
          <div>
            <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider block mb-2">Instructions</label>
            {form.instructions.map((step, i) => (
              <div key={i} className="flex items-start gap-2 mb-2">
                ...
                <span className="text-sm text-gray-700 flex-1 leading-relaxed">{step}</span>
                <button type="button" onClick={() => set('instructions', form.instructions.filter((_, j) => j !== i))} className="text-gray-400 hover:text-red-500 mt-0.5">
                  <Trash2 size={13} />
                </button>
              </div>
            ))}
            <div className="flex gap-2 mt-2">
              ...
              <button type="button" onClick={addStep} className="p-2 bg-orange-500 text-white rounded-xl hover:bg-orange-600 transition-colors self-start flex-shrink-0">
                <Plus size={16} />
              </button>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="flex gap-2">
              ...
              <button type="button" onClick={addTag} className="p-2 bg-gray-500 text-white rounded-xl hover:bg-gray-600 transition-colors flex-shrink-0">
                <Plus size={16} />
              </button>
            </div>
          </div>
          ...
          <div className="pb-4">
            <button
              type="submit"
              className="w-full bg-orange-500 hover:bg-orange-600 text-white font-semibold py-3 rounded-xl transition-colors text-sm shadow-md"
            >
